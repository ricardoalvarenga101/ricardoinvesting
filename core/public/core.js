...
function createTrigger() {
  deleteTrigger();
  ScriptApp.newTrigger("updateCotation").timeBased().everyMinutes(30).create();
...
function getEvolutionRentability() {
  ...
  const sumVariation = Guia.getRange("L13").getValue();
  const total = sumVariation;
  ...
  if (days.includes(new Date().getDay())) {
    if (now == "09" || now === 9 || now === 10) {
      ...
      console.log(`Registrando variação do dia anterior -> ${total}`);
      Cell.setValue(total);
...
function updateCotation() {
  ...
  const cotations = GuideCotation.getRange(
    `A2:C${totalRegister + 1}`
  ).getValues();
  ...
  GuideCotation.getRange(`D2:D${offlineCotations.length + 1}`).setValues(
    offlineCotations
  );
...
function updateCotationManual() {
  const outputClose = HtmlService.createHtmlOutput(
    "<script>google.script.host.close();</script>"
  );
  const ui = SpreadsheetApp.getUi();
  ui.showSidebar(
    HtmlService.createHtmlOutputFromFile(
      "@ricardoinvesting-cotation-html"
...
function updatePM() {
  ...
  const Sheet = SpreadsheetApp.getActiveSpreadsheet();
  const GuideDinamicConsolid = Sheet.getSheetByName(
    ABAS.TABELA_DINAMICA_CONSOLIDADO
  );
  ...
  for (let i = 0; i < listYears.length; i++) {
    ...
    const yCONSOLID_last = calculateAmmountIRPFFull(
      listYears[i] - 1,
      uuid,
      false
    );
  ...
  const GuideTDConsolidado = Sheet.getSheetByName(
    ABAS.TABELA_DINAMICA_CONSOLIDADO
  );
  GuideTDConsolidado.getRange("V13").setValue(uuid);
}
...
function updatePMManual() {
  ...
  const ui = SpreadsheetApp.getUi();
  ui.showSidebar(
    HtmlService.createHtmlOutputFromFile("@ricardoinvesting-pm-html").setTitle(
      "Recalculando Preço Médio"
    )
  );
...
function updatePerformance(value, shortDate) {
  ...
  console.log(`Registrando fechamento de mês -> ${value}`);
}
...
function checkVersion(version, trigger) {
  try {
    ...
    const response = UrlFetchApp.fetch(
      "https://bombolao-v2.rj.r.appspot.com/core/api/version/message"
    );
    ...
    for (let i = initialLine; i <= (dataRows.length - 1); i++) {
      ...
      if (yearAnalysis === year) {
        ...
        switch (dataRows[i][2]) {
          ...
          case "Aluguel":
            if (!isExternal) {
              tickers[rowTicker].rentals.push(dataRows[i][10]);
              tickers[rowTicker].amountRental += dataRows[i][10];
            }
            break;
        }
...
function irReportLoadingData(year = 2024, historyCurrent = false, historyPast = true) {
  ...
  const rentals = composeRentalsToYear(provents, database)
  return { itensWallletFiltered, provents, sells, bonifications, bonificationsWithFractions, rentals };
...
function composeRentalsToYear(provents, database) {
  let rentalsModify = {}
  const tickerList = Object.keys(provents)
  tickerList.forEach(item => {
    if (item !== "external") {
      if (provents[item].amountRental) {
        let accumulated = 0
        rentalsModify[item] = {
          amount: provents[item].amountRental + accumulated,
          cnpj: database[item].document_number_principal || database[item].document_number_admin,
          name: database[item].name
        }
      }
